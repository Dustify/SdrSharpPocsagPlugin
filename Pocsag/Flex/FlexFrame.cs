            ...
            for (var i = 0; i < this.Words.Count; i++)
            {
                try
                {
                    ...
                    if (i >= this.VectorStart && i < this.VectorStart + this.Addresses.Count)
                    {
                        ...
                        if (result.ParityError || result.BchErrors)
                        {
                            errors = true;
                        }
                        ...
                        var relevantAddressResult = this.Addresses[i - (int)this.VectorStart];

                        var relevantAddress = relevantAddressResult.Address;

                        if (relevantAddressResult.Result.ParityError || relevantAddressResult.Result.BchErrors)
                        {
                            errors = true;
                        }
